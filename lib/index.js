/*
 * mhg-js
 * https://github.com/JEFworks/mhg-js
 *
...
var  mhgjs;
module.exports = mhgjs = function(opts){
    this.el = opts.el;
    
    var mhg = new MHG();
    var res = mhg.mhg_test(opts.v, opts.N, opts.K, opts.L, opts.X);
    //console.log(res.mhg);
    //console.log(res.matrix);
    //console.log(res.pvalue);
	...
	// Add the p-value to the x axis.
	    .append("text")
	    .attr("x", x(res.threshold))
	    .attr("y", -y(d3.min(res.mhg)) - 5)
	    .style('fill', 'red')
	...
	var bars = opts.v.map(function(d, i) {
	    if (d > 0) {
		return {x: i + 1, y: 1};
	    }
	...
	this.el.textContent = 'p-value: ' + res.pvalue;
    }
